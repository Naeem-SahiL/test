...
module.exports = function(app, upload,unlinkAsync){
    app.post("/insertDependantRenewalApplication",upload.fields(
        [
            {name:'photo'},
            {name:'dependantVisa'},
            {name:'passportCopy'},
        ...
        (req, res) =>{
            try{
                ...
                const query = "INSERT INTO dependant_renewal_application(photo,\
                    certified,\
                    dependantVisa,\
                    passportCopy,\
                    applicationFee,\
    ...
    app.put("/updateDependantRenewalApplicationDataById", upload.fields(
        [
        {name:'photo'},
        {name:'dependantVisa'},
        {name:'passportCopy'},
    ...
    (req, res) =>{
        try{
            ...
            const query = "UPDATE dependant_renewal_application SET photo=?,\
                certified=?,\
                dependantVisa=?,\
                passportCopy=?,\
                applicationFee=?,\
...
app.delete("/deleteDependantRenewalApplicationDataById/:appId",
(req, res) => {
    try {
        console.log("hy")
        var appId = req.params.appId
        db1.query("SELECT * FROM dependant_renewal_application WHERE app_id=?",[appId],(err,result)=>{
            if(err)
            {
                console.log("db error ->", err)
                ...
                try{
                    ...
                    db1.query("DELETE FROM dependant_renewal_application WHERE app_id = ?",[appId],(err,result)=>
                    {
                        if (err) {
                            console.log("db error ->", err)
                            res.status(202).send("Error in deleting data in database")
...
app.get("/getDependantRenewalApplicationData", 
(req, res) => {
    try {
        db1.query("SELECT dependant_renewal_application.* , users.username FROM `dependant_renewal_application` LEFT JOIN users on users.user_id = dependant_renewal_application.created_by ORDER BY app_id DESC ",
            (err, result) => {
                if (err) {
...
app.get("/getDependantRenewalApplicationById/:appId", 
        (req, res) => {
            const appId = req.params.appId
            console.log(appId)
            try {
                db1.query("SELECT * FROM  dependant_renewal_application WHERE app_id = ?",[appId],
                    (err, result) => {
                        if (err) { 
                            console.log(err)
                            res.status(202).send("Error in fetching data from database")
 ...
 //submit
app.post("/submitDependantRenewalApplicationData",
(req, res) => {
    try {
        const appId = req.body.appId
        const startDate = req.body.startDate
        console.log(appId)
        db1.query("INSERT INTO _dependant_renewal_application_status(app_id,submitted) VALUES (?,?);",[appId, startDate],
            (err, result) => {
                if (err) {
                    console.log(err)
                    ...
                    console.log("result")
                    db1.query("UPDATE dependant_renewal_application SET submitted = 1 WHERE app_id = ?", [appId], (err, result) =>{
                        if (err) {
                            console.log(err)
